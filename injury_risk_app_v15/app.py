...
# ==== Interface Streamlit ====
st.set_page_config(page_title="Risque de Blessure - Radar & PDF", layout="wide")
st.title("🧠 Risque de Blessure - Radar & Rapport PDF")

with st.form("formulaire"):
    col1, col2, col3 = st.columns(3)
    with col1:
        dorsiflexion = st.slider("Amplitude dorsiflexion (°)", 0, 40, 20)
...
if submit:
    ...
    features = np.array([dorsiflexion, adducteur_strength, sprint_time, squat_1RM, vertical_jump, charge_var, fatigue, sommeil, blessure_bin]).reshape(1, -1)
    ...
    champs = [
        ...
        ("Adducteurs (kg)", adducteur_strength),
        ("Sprint 10m (s)", sprint_time),
